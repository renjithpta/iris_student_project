...
function printHelp () {
  ...
  echo "	addorg4.sh up"
  echo "	addorg4.sh up -c irischannel -s couchdb"
...
function generateorg4Definition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  configtxgen -printOrg Org4MSP > ../organizations/peerOrganizations/org4.irisauth.com/org4.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function org4Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/org4.irisauth.com/peers/peer0.org4.irisauth.com/
  fi
...
function addorg4 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org4.irisauth.com" ]; then
    generateorg4

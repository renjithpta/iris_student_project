...
function printHelp () {
  ...
  echo "    -c <channel name> - test network channel name (defaults to \"irischannel\")"
  ...
  echo "	addOrg3.sh up"
  echo "	addOrg3.sh up -c irischannel -s couchdb"
...
function generateOrg3Definition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3.irisauth.com/org3.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function Org3Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/org3.irisauth.com/peers/peer0.org3.irisauth.com/
  fi
...
function addOrg3 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org3.irisauth.com" ]; then
    generateOrg3
...
#default for delay
CLI_DELAY=3
# channel name defaults to "irischannel"
CHANNEL_NAME="irischannel"

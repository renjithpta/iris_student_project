...
function printHelp () {
  ...
  echo "	addorg5.sh up"
  echo "	addorg5.sh up -c irischannel -s couchdb"
...
function generateorg5Definition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  configtxgen -printOrg Org5MSP > ../organizations/peerOrganizations/org5.irisauth.com/org5.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function org5Up () {
  ...
  if [ "$CONTAINER_CLI" == "podman" ]; then
    cp ../podman/core.yaml ../../organizations/peerOrganizations/org5.irisauth.com/peers/peer0.org5.irisauth.com/
  fi
...
function addorg5 () {
  ...
  if [ ! -d "../organizations/peerOrganizations/org5.irisauth.com" ]; then
    generateorg5

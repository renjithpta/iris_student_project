...
set -x
jq -s '.[0] * {"channel_group":{"groups":{"Application":{"groups": {"Org5MSP":.[1]}}}}}' config.json ./organizations/peerOrganizations/org5.irisauth.com/org5.json > modified_config.json
{ set +x; } 2>/dev/null
...
setGlobals 2
set -x
peer channel update -f org5_update_in_envelope.pb -c ${CHANNEL_NAME} -o orderer.irisauth.com:7050 --ordererTLSHostnameOverride orderer.irisauth.com --tls --cafile "$ORDERER_CA"
{ set +x; } 2>/dev/null
